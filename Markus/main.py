...
def main():
    ...
    for i in range(len(time_array)-1):
        ...
        if (np.min(distances_to_car_lane_1) <= CRITICAL_DISTANCE) or (np.min(distances_to_car_lane_2) <= CRITICAL_DISTANCE):
            ...
            # if (np.sum(measured_x_lane_1 > x_car) > int(8*N/X_MAX) and np.sum(measured_x_lane_2 > x_car) > int(8*N/X_MAX)): # Ma distance for the lookahead to 8m
            ...
            x_ref_array, y_ref_array, theta_ref_array = mpc_control.generate_trajectory(x_car, y_car, theta, measured_x_lane_1, measured_y_lane_1, measured_x_lane_2, measured_y_lane_2, v=CAR_SPEED, dt=dt, horizon=HORIZON)


            w_s = mpc_control.solve_mpc(states[i], x_ref_array, y_ref_array, L=CAR_LENGTH, v=CAR_SPEED, dt=dt, horizon=HORIZON)
