...
def camera_sensor(car_x, car_y,
                  x_lane_1, y_lane_1,
                  x_lane_2, y_lane_2):
    """
    Returns noisy camera measurements of the nearest point on each lane.
    """

    def measure_lane(x_lane, y_lane):
        dx = x_lane - car_x
        dy = y_lane - car_y
        ...
        if not np.any(mask):
            return None  # No points detected
    ...
    # Compute measurements for each lane
    lane_1_measurement =measure_lane(x_lane_1, y_lane_1)
    lane_2_measurement =measure_lane(x_lane_2, y_lane_2)
